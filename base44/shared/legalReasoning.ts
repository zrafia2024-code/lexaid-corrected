...
export function runReasoning(categoryId, answers, dynamicFactors = null) {
  const cat = CATEGORIES[categoryId] || CATEGORIES.other;
  const rawFactors = (Array.isArray(dynamicFactors) && dynamicFactors.length > 0) ? dynamicFactors : cat.factors;
  const factors = rawFactors.map((f, i) => ({
    key: f.key || f.id || `factor_${i}`,
    label: f.label || f.question || `Factor ${i + 1}`,
    urduLabel: f.urduLabel || f.urduQuestion || f.label || `پہلو ${i + 1}`,
    question: f.question || f.label,
    urduQuestion: f.urduQuestion || f.urduLabel,
    weight: typeof f.weight === "number" ? f.weight : 15,
    favorable: Array.isArray(f.favorable) ? f.favorable.map((x) => String(x).toLowerCase()) : ["yes"],
  }));
