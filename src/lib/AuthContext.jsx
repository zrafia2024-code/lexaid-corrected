...
// Migrate guest-created cases and documents into a newly authenticated user account
export const migrateGuestData = async (targetUserId) => {
  try {
    const guestId = localStorage.getItem("lexaid_guest_id");
    if (!guestId || guestId === targetUserId) return;

    const guestCasesKey = `lexaid_user_${guestId}_cases`;
    const targetCasesKey = `lexaid_user_${targetUserId}_cases`;
    const guestDocsKey = `lexaid_user_${guestId}_documents`;
    const targetDocsKey = `lexaid_user_${targetUserId}_documents`;

    const rawGuestCases = localStorage.getItem(guestCasesKey);
    if (rawGuestCases) {
      const guestCases = JSON.parse(rawGuestCases);
      const rawTargetCases = localStorage.getItem(targetCasesKey);
      const targetCases = rawTargetCases ? JSON.parse(rawTargetCases) : [];
      const mergedMap = new Map();
      targetCases.forEach((c) => mergedMap.set(c.id, c));
      guestCases.forEach((c) => mergedMap.set(c.id, { ...c, user_id: targetUserId }));
      localStorage.setItem(targetCasesKey, JSON.stringify(Array.from(mergedMap.values())));
    }

    const rawGuestDocs = localStorage.getItem(guestDocsKey);
    if (rawGuestDocs) {
      const guestDocs = JSON.parse(rawGuestDocs);
      const rawTargetDocs = localStorage.getItem(targetDocsKey);
      const targetDocs = rawTargetDocs ? JSON.parse(rawTargetDocs) : [];
      const mergedMap = new Map();
      targetDocs.forEach((d) => mergedMap.set(d.id, d));
      guestDocs.forEach((d) => mergedMap.set(d.id, { ...d, user_id: targetUserId }));
      localStorage.setItem(targetDocsKey, JSON.stringify(Array.from(mergedMap.values())));
    }

    await fetch("/api/migrate-guest-cases", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ guestId, targetUserId }),
    });
  } catch (err) {
...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    // Clean up legacy unscoped history stores and mock users from localStorage
    try {
      ...
      try {
        if (isSupabaseConfigured && supabase) {
          ...
          if (session?.user) {
            ...
            localStorage.setItem('lexaid_local_user', JSON.stringify(activeUser));
  ...
  const login = async (email, password) => {
    ...
    if (isSupabaseConfigured && supabase) {
      try {
        ...
        setUser(loggedUser);
        setIsAuthenticated(true);
        localStorage.setItem('lexaid_local_user', JSON.stringify(loggedUser));
        // Automatically save/update user in public.pwa_users table
        await syncUserToPwaUsers(loggedUser);
        migrateGuestData(loggedUser.id);
        return loggedUser;
      } catch (err) {
        throw err;
      }
    ...
    if (isSupabaseConfigured && supabase) {
      try {
        ...
        if (data?.user) {
          ...
          setUser(newUser);
          setIsAuthenticated(true);
          localStorage.setItem('lexaid_local_user', JSON.stringify(newUser));
