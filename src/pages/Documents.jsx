...
import { FileText, Upload, CheckCircle2, AlertTriangle, Calendar, HelpCircle, Sparkles, Trash2 } from "lucide-react";
...
export default function Documents() {
  ...
  const loadDocs = async () => {
    try {
      const list = await base44.entities.LegalDocument.list("-created_date", 50);
      setDocuments(list || []);
    } catch (err) {
  ...
  const loadCases = async () => {
    try {
      const list = await base44.entities.LegalCase.list("-created_date", 50);
      setCases(list || []);
    } catch (err) {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      setIsDeleting(true);
      await base44.entities.LegalDocument.delete(docToDelete.id);
    ...
    <div className="space-y-8 max-w-4xl mx-auto">
                ...
                <div className="flex items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    ...
                    <button
                      ...
                      title={isUr ? "دستاویز حذف کریں" : "Delete document"}
                      aria-label={isUr ? "دستاویز حذف کریں" : "Delete document"}
                    >
      ...
      <AlertDialog open={Boolean(docToDelete)} onOpenChange={(open) => !open && setDocToDelete(null)}>
        <AlertDialogContent className="sm:max-w-md">
          <AlertDialogHeader>
            <AlertDialogTitle className={isUr ? "font-urdu" : ""}>
              {isUr ? "دستاویز کو حذف کریں" : "Delete Document"}
            </AlertDialogTitle>
            <AlertDialogDescription className={isUr ? "font-urdu text-sm" : ""}>
              {isUr
                ? "کیا آپ واقعی اس دستاویز کو اپنی ہسٹری سے حذف کرنا چاہتے ہیں؟"
          ...
          <AlertDialogFooter className="gap-2 sm:gap-0">
            ...
            >
              {isDeleting ? (isUr ? "حذف ہو رہا ہے..." : "Deleting...") : (isUr ? "حذف کریں" : "Delete")}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
