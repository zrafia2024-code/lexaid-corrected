...
export default function Assistant() {
  ...
  // Helper to ensure all displayed issues are formatted in natural, clear declarative sentences without "Whether"
  const formatDisplayIssue = (text, isUrdu) => {
  ...
  const handleAssess = async () => {
    try {
      ...
      if (!finalCaseId) {
        try {
          const created = await base44.entities.LegalCase.create({
            title: description.slice(0, 60),
            description: description.trim(),
            language: lang,
            category: understanding?.category || "other",
            answers: JSON.stringify(answers),
            assessment: JSON.stringify(a),
            references: JSON.stringify(r || []),
            status: "assessed",
          });
          if (created?.id) finalCaseId = created.id;
        } catch (e) {
          console.warn("Fallback case creation error:", e);
        }
      }
  ...
  const handleSaveCase = async () => {
    try {
      ...
      console.warn("Save case error:", e);
    }
  };
